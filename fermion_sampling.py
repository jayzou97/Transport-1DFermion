...
def dpp_new(U,N,L):
    ...
    prob = p / sum(p)   
    Xn = np.random.choice(L,p = prob)  # sample Xn
    e = K[:,Xn] / np.linalg.norm(K[:,Xn])  # set e1
    X = [0 for i in range(N)]
    X[N-1] = Xn
    W = K
    for i in range(N-1,0,-1):
        p = np.zeros(L)
        for j in range(L):
            W[:,j] = W[:,j] - np.dot(e.conj(),W[:,j]) * e  # subtract only projection on the e of the last step
            p[j] = np.square(np.linalg.norm(W[:,j]))    # P_i = i(x = j)
        prob = abs(p) / sum(abs(p))    
        X[i-1] = np.random.choice(L,p = prob)
        wi = W[:,X[i-1]]
        e = wi / np.linalg.norm(wi)
    m = [1 if i in X else 0 for i in range(L)]           # generate m vector with x
    return np.array(m)
...
def sampling_new(u,N,L):
    v = np.random.permutation(N)
    U = u[:, v]
    sampl = np.zeros(L, dtype=bool)
    ground_set = np.arange(L)
    avail = np.ones(L, dtype=bool)

    p = np.square(np.abs(U[:,0]))
    prob = p / sum(p)
    x_new = np.random.choice(L,p = prob)
    sampl[x_new] = True
    avail[x_new] = False
    U_x = U[sampl]
    ...
    for i in range(1,N):
        u_new = U[x_new]
        # Guass elimination to form the new U_x
        if i == 1:
            pass
        else:
            for j in range(i-1):
                ratio = u_new[j] / U_x[j,j]
                u_new[j:] = u_new[j:] - ratio * U_x[j,j:]
            U_x = np.vstack((U_x, u_new))
        # find the normal vector of the subspace {row(x1), row{x2},... , row{x(i-1)}}
        a = np.zeros(i+1)
        a[-1] = 1
        for k in range(i):
            l = i - k - 1
            a[l] = - np.dot(a[l+1:], U_x[l, l+1:i+1]) / U_x[l,l]
        
        p0 = np.dot(U[avail][:, :i+1],a) 
        p = np.square(np.abs(p0))
        prob = p / sum(p)
        x_new = np.random.choice(ground_set[avail],p = prob)
        sampl[x_new] = True
        avail[x_new] = False          # choose position wrt p  
...
def dppy_algo(U,N,L): 
    ...
    # print(DPP.list_of_samples[0])
    x = DPP.list_of_samples[0]
    m = [1 if i in x else 0 for i in range(L)]
    return np.array(m)

def dppy_kernel(K,N,L): 
    DPP = FiniteDPP(kernel_type='correlation',
                            projection=False,
                            **{'K': K})
    DPP.sample_exact(mode='GS')
...
def sample_position(loop,method,U,N,L):
    ...
    if method == "dpp":
        ...
        for _ in range(loop):
            result = result + dpp_new(U,N,L)
    elif method == "dppy":
        for _ in range(loop):
            result = result + dppy_algo(U,N,L)
    elif method == "sampling_new":
        for _ in range(loop):
            result = result + sampling_new(U.T,N,L)
...
def sample_state(loop,method,U,N,L):
    ...
    if method == "dpp":
        ...
        for _ in range(loop):
            result.append(bit2int(dpp_new(U,N,L)))
    elif method == "dppy":
        for _ in range(loop):
            result.append(bit2int(dppy_algo(U,N,L)))
    elif method == "sampling_new":
        for _ in range(loop):
            result.append(bit2int(sampling_new(U.T,N,L)))
    elif method == "sampling":
        for _ in range(loop):
...
if __name__ == "__main__":
    loop = int(sys.argv[4])    # number of samples
    N = int(sys.argv[3])      # num of particle
    L =  int(sys.argv[2])     # length of the system
    method_input = sys.argv[1]   # sampling method
    # loop = 10000
    # N = 2
    # L = 4
    # generate a complex unitary
    '''
    U = unitary_group.rvs(L)
    U = U[0:N,:]
    '''
    # generate a real unitary
    U, _ = qr(np.random.randn(L, N), mode='economic')
    U = U.T
    ...
    # sample_position(loop,"dpp_new",U,N,L)
    # sample_position(loop,"sampling",U,N,L)
    # sample_position(loop,"sampling_new",U,N,L)
    # sample_position(loop,"dppy",U,N,L)
    sample_position(loop,method_input,U,N,L)

    '''
    count0 = np.array(sample_state(loop,"dppy",U,N,L))
    count0 = count0[count0 != 0]    
    count1 = np.array(sample_state(loop,"sampling_new",U,N,L))
